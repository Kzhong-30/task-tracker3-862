...
function App() {
  ...
  async function clearCompleted() {
    ...
    await Promise.all(completedTasks.map((task) => fetch(`${API_URL}/api/tasks/${task._id}`, { method: 'DELETE' })));
    fetchTasks();
  }
      ...
      <div className='task-card'>
        ...
        <form onSubmit={addTask} className='add-task-form'>
          <div className='input-group'>
            <input ref={inputRef} value={title} onChange={(e) => setTitle(e.target.value)} placeholder='What needs to be done? (Press Enter to add)' className='task-input' />
        ...
        <div className='search-bar'>
          <input type='text' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder='🔍 Search tasks...' className='search-input' />
        </div>
        ...
        {loading ? (
          ...
          <div className='empty-state'>
            <div className='empty-icon'>{searchQuery ? '🔍' : filter === 'all' ? '📝' : filter === 'active' ? '🎉' : '✅'}</div>
            <p className='empty-message'>{searchQuery ? `No tasks matching "${searchQuery}"` : filter === 'all' ? 'No tasks yet. Add one above!' : filter === 'active' ? 'No active tasks. Great job! 🎉' : 'No completed tasks yet.'}</p>
          </div>
        ) : (
          <ul className='task-list'>
            {filteredTasks.map((task, index) => (
              <li key={task._id} className={`task-item ${task.completed ? 'completed-item' : ''}`} style={{ animationDelay: `${index * 0.05}s` }}>
                <div className='task-left'>
                  <input type='checkbox' checked={task.completed} onChange={() => toggleTask(task._id)} className='task-checkbox' />
                  {editingId === task._id ? (
                    <input
                      type='text'
                      value={editText}
                    ...
                    <div className='task-text-container'>
                      <div className='task-title-row'>
                        <span className={task.completed ? 'task-text completed' : 'task-text'} onClick={() => toggleTask(task._id)}>
                          {task.title}
                        </span>
                        {task.priority && (
                      ...
                      {task.createdAt && <span className='task-time'>{getTimeAgo(task.createdAt)}</span>}
                    </div>
                  )}
                </div>
                <div className='task-actions'>
                  {!task.completed && editingId !== task._id && (
                    <button onClick={() => startEdit(task)} className='edit-button' title='Edit task'>
                      ✏️
                    </button>
                  )}
